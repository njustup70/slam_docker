...
public:
  ...
  void pub_odom_func(IMUST &xc, ros::Time pcl_time)
  {
    ...
    ros::Time ct = pcl_time;
    br.sendTransform(tf::StampedTransform(transform, ct, "/camera_init", "/aft_mapped"));
  }

  void pub_localtraj(PLV(3) &pwld, double jour, IMUST &x_curr, int cur_session, pcl::PointCloud<PointType> &pcl_path, ros::Time pcl_time)
  {
    pub_odom_func(x_curr, pcl_time);
...
public:
  ...
  int initialization(deque<sensor_msgs::Imu::Ptr> &imus, Eigen::MatrixXd &hess, LidarFactor &voxhess, PLV(3) &pwld, pcl::PointCloud<PointType>::Ptr pcl_curr)
  {
    ...
    ros::Time init_time = ros::Time(odom_ekf.pcl_beg_time); 
    ResultOutput::instance().pub_localtraj(pwld, 0, x_curr, sessionNames.size()-1, pcl_path, init_time);
  ...
  void thd_odometry_localmapping(ros::NodeHandle &n)
  {
    ...
    while(n.ok())
    {
      ...
      ros::Time pcl_time; 
      if(!sync_packages(pcl_curr, imus, odom_ekf, pcl_time))
      {
      ...
      else
      {
        ...
        pwld.clear();
        pvec_update(pptr, x_curr, pwld);
        ResultOutput::instance().pub_localtraj(pwld, jour, x_curr, sessionNames.size()-1, pcl_path, pcl_time);
