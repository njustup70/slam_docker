 ...
 * 3. Detect the radius-search-based loop closure, and add them to the pose
 * graph
 * @version 0.1
 * @date 2022-03-11
 *
...
double translationAccumulated =
    1000000.0; // large value means must add the first given frame.
...
void laserCloudFullResHandler(
    const sensor_msgs::PointCloud2ConstPtr &_laserCloudFullRes) {
  mBuf.lock();
...
void pubPath(void) {
  ...
  // node_idx++) // -1 is just delayed visualization (because sometimes mutexed
  // while adding(push_back) a new one)
  for (int node_idx = 0; node_idx < recentIdxUpdated;
       node_idx++) // -1 is just delayed visualization (because sometimes
                   // mutexed while adding(push_back) a new one)
  {
    const Pose6D &pose_est =
        keyframePosesUpdated.at(node_idx); // upodated poses
    // const gtsam::Pose3& pose_est =
    // isamCurrentEstimate.at<gtsam::Pose3>(node_idx);

    nav_msgs::Odometry odomAftPGOthis;
    odomAftPGOthis.header.frame_id = "camera_init";
    odomAftPGOthis.child_frame_id = "/aft_pgo";
    odomAftPGOthis.header.stamp =
        ros::Time().fromSec(keyframeTimes.at(node_idx));
...
gtsam::Pose3 doICPVirtualRelative(int _loop_kf_idx, int _curr_kf_idx) {
  ...
  loopFindNearKeyframes(targetKeyframeCloud, _loop_kf_idx,
  ...
  // float loopFitnessScoreThreshold = 0.3; // user parameter but fixed low
  // value is safe.
...
vector2pc(const std::vector<Pose6D> vectorPose6d) {
...
bool detectLoopClosureDistance(int *loopKeyCur, int *loopKeyPre) {
  ...
  for (int i = 0; i < pointSearchIndLoop.size(); ++i) {
    ...
    if (abs(keyframeTimes[id] - keyframeTimes[*loopKeyCur]) >
...
void performRSLoopClosure(void) {
  ...
  if (detectLoopClosureDistance(&loopKeyCur, &loopKeyPre)) {
    cout << "Loop detected! - between " << loopKeyPre << " and " << loopKeyCur
         << "" << endl;
    mBuf.lock();
  ...
  while (1) {
    while (!scLoopICPBuf.empty()) {
      if (scLoopICPBuf.size() > 30) {
        ROS_WARN(
            "Too many loop clousre candidates to be ICPed is waiting ... Do "
            "process_lcd less frequently (adjust loopClosureFrequency)");
      }
...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("save_directory", save_directory,
                        "/"); // pose assignment every k m move
  ...
  nh.param<double>("sc_max_radius", scMaximumRadius,
                   80.0); // 80 is recommended for outdoor, and lower (ex, 20,
  ...
  nh.param<double>("mapviz_filter_size", mapVizFilterSize,
                   0.4); // pose assignment every k frames
  ...
  pubLoopScanLocalRegisted =
      nh.advertise<sensor_msgs::PointCloud2>("/loop_scan_local_registed", 100);
  ...
  std::thread isam_update{
      process_isam}; // if you want to call less isam2 run (for saving redundant
                     // computations and no real-time visulization is required),
                     // uncommment this and comment all the above runisam2opt
                     // when node is added.

  std::thread viz_map{process_viz_map}; // visualization - map (low frequency
                                        // because it is heavy)
  // std::thread viz_path {process_viz_path}; // visualization - path (high
  // frequency)
